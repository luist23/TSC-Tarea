...
void inversa(Matrix M, Matrix &R){
    //cout<<"calculando el Determinante \n";
    float det = determinant(M);
    //cout<<"Determinante: "<< det << '\n';
    if(det == 0.0){
        //cout<<"Dterminante es igual a 'cero' por lo tanto 'NO existe solucion'";
        exit(EXIT_FAILURE);
    }
    else{
        Matrix cof;
        //cout<<"Calculando la Matrix de cofactores \n";
        cofactors(M,cof);
        //showMatrix(cof);
        //cout<<"Transponiendo Matrix de Cofactores \n";
        Matrix T;
        //showMatrix(T);
        transpose(cof,T);
        productRealMatrix(1.0/det,T,R);
    }
    //return 1;
}
